document.addEventListener('DOMContentLoaded', () => {
    ...
    let randomNumbers = generateUniqueRandomNumbers(6);
    ...
    btns.forEach((item) => {//Використовується метод forEach для проходження по кожному елементу в колекції btns. btns - це, імовірно, колекція кнопок.
        ...
        clueBtn.addEventListener('click', () => {
            ...
            if (itemText === "DEL") {
                ...
                if (currentValue.length < 6 && !currentValue.includes(itemText)) {//Перевіряється, що довжина значення display менша за 4 та це значення не містить текст кнопки.
